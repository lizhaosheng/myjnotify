...
public class JNotify_linux
{
	...
	static
	{
		try
		{
			String path = JNotifyConstant.LIB_CLASS_PATH + "/libjnotify_linux_64.so";
			System.out.println("try to load '" + path + "' from classpath");
			File lib = Util.getTempLib(path, ".so");
			System.load(lib.getAbsolutePath());
		}
		catch (UnsatisfiedLinkError e)
		{
			System.err.println("load libjnotify_linux_64.so failed!\n" + e.getMessage());
			try
			{
				String path = JNotifyConstant.LIB_CLASS_PATH + "/libjnotify_linux.so";
				System.out.println("try to load '" + path + "' from classpath");
				File lib = Util.getTempLib(path, ".so");
				System.load(lib.getAbsolutePath());
			}
			...
			{
				System.err.println("load libjnotify_linux.so failed!\n" + e2.getMessage());
				throw e2;
